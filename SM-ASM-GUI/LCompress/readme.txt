...
______________________________________________________________________
 ...
 Moon Another Story RPG, Sailor Moon R, Marvelous, Earthbound, Kirby
 Super Star, Kirby's Adventure (NES), Pokemon Gold & Silver (GB),
...
Version 1.90 December 25, 2021

-added a new format used in Marvelous (LC_LZ20).
-added a couple options for ROM expansion with SA-1 and SDD-1
 (LC_48_EXPCHIP and LC_64_EXPCHIP).
-added LC_SUPERFXROM and LC_SUPERFXROM_2 for Super FX address
 conversion.
-added a couple options for the IPS functions (LC_IPS_ALLOW_MENU,
 LC_IPS_ADD_HEADER, LC_IPS_REMOVE_HEADER).
-added a new LunarSaveRAMFileEx() function so a flags value could be
 added, and depreciated the original LunarSaveRAMFile() function.
-added an option to file functions for using UTF-8 file names
 (LC_UTF8).
-added support for loading the DLL from unicode paths when running on
 a unicode OS.
-added builds of the sample programs that support unicode
 filenames/paths when running on a unicode OS.


Version 1.81 May 16, 2017

-fixed a crash bug that can occur when decompressing from a RAM file.
 Most formats would never have triggered it, but LC_LZ12 (Gradius 3)
 definitely was.


Version 1.80 February 9, 2015

-added a new format used in Kirby Super Star, Kirby's Adventure (NES),
 and Earthbound (LC_LZ19).
-fixed a crash bug that occured with recompression for format LC_LZ12
 (Gradius 3).  Thanks goes out to imamelia for reporting this.
-fixed a crash bug that occured only in the x64 DLL with recompression
 for format LC_LZ3.  Thanks goes out to imamelia for reporting this.
...
______________________________________________________________________
 ...
 ¹Does not apply to any source code files that are included as part of
  this package, which you may modify, distribute, and use as needed.
